#!/usr/bin/env python3
import subprocess, sys, platform, shutil, threading, unittest, psutil, socket
...
def gather_system_info(logfile):
    ...
    for iface, addrs in net.items():
        for addr in addrs:
            if addr.family == socket.AF_INET:  # Use constant for readability
...
class WindowsUpdater(BaseUpdater):
    ...
    def update_firmware(self, logfile):
        ...
        self.run_command(
            "powershell -Command \"Install-Module -Name FirmwareUpdate -Force; "
