...
def main():
    log_filename = f"update_log_{datetime.now().strftime('%Y-%m-%d')}.txt" #Makes a log file
    try:
        # Write system info at the top only once
        gather_system_info(log_filename) #Parse the filename
        manager = UpdaterManager() #Make an instance of manager
        os_name = manager.detect_os() #Varible record of the spesific OS
        log_message("INFO", f"Detected OS: {os_name}", log_filename) #MSG
        manager.run(log_filename) #Run it!
